...
function mergeConfig(customize, defaults) {
  const config = Object.assign(Object.assign({}, defaults), customize);
  if (defaults.headers && customize.headers) {
    config.headers = Object.assign(Object.assign({}, defaults.headers), customize.headers);
  }
  return config;
}
  ...
  if (isConnected && !isHidden) {
    ...
    if (cancelToken) {
      cancelToken.promise.then(() => {
        const index2 = callbackPools.indexOf(callback);
        if (index2 !== -1) {
          callbackPools.splice(index2, 1);
        }
      });
    }
  }
...
class LifeCycle {
  ...
  _onSend(options) {
    this.Listeners.onSend.forEach((f2) => {
      f2(options);
    });
  ...
  _send(data, options) {
    return new Promise((resolve, reject) => {
      ...
      data.success = (res) => {
        completed = true;
        this._response(res, options).then(resolve, reject);
      };
      data.fail = (res) => {
        if (timeoutHandle === 0) {
        ...
        if (cancelToken && cancelToken.isCancelled()) {
          ...
          Promise.resolve().then(() => options.retry(data, res)).then(
            (retryData) => {
              this._send(retryData, options).then(resolve, reject);
            },
            (reason) => {
          ...
          this._send(data, options).then(resolve, reject);
          return;
        }
        completed = true;
  ...
  _response(res, options) {
    this.Listeners.onResponse.forEach((f2) => {
      f2(res, options);
    });
  ...
  _onFail(res, options) {
    this.Listeners.onRejected.forEach((f2) => {
      f2(res, options);
    });
    return Promise.reject(res);
  }
  _onComplete(res, options) {
    this.Listeners.onComplete.forEach((f2) => {
      f2(res, options);
    });
  }
  _onAbort(reason, options) {
    this.Listeners.onAbort.forEach((f2) => {
      f2(reason, options);
    });
  }
}
if (!Promise.prototype.finally) {
  Promise.prototype.finally = function(onfinally) {
    if (onfinally) {
      ...
      return this.then((value) => P.resolve(onfinally()).then(() => value), (reason) => P.resolve(onfinally()).then(() => {
        throw reason;
      }));
    } else {
      return this;
...
class Http extends LifeCycle {
  constructor(config, request, listeners) {
    super(
      request || wx.request,
      config || { transformSend: transformRequestSendDefault },
      listeners
    );
  }
  request() {
...
const REQUEST = /* @__PURE__ */ new Http();
